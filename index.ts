 ...
 */
export function callback(auditCommand: string, auditLevel: AuditLevel, exceptionIds: number[], filter: boolean): void {
  ...
  if (audit.stderr) {
    audit.stderr.on('close', () => handleFinish(jsonBuffer, auditLevel, exceptionIds, filter));
    // stderr
